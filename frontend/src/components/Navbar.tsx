      ...
      <div className="flex items-center justify-between px-4 md:px-10 py-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex gap-8 font-semibold text-gray-700 text-lg">
            <Link
              to="/"
              className="font-bold hover:text-black border-b-2 border-red-500 pb-1"
            >
              HOME
            </Link>
            <Link
        ...
        <div className="flex items-center gap-4 md:gap-7">
          ...
          {user && (
            <Link to="/wishlist" className="relative">
              <Heart className="w-6 h-6 cursor-pointer sm:block" />
              {wishlistCount > 0 && (
          ...
          {user?.role === "admin" && (
            <Link
              to="/admin"
              className="hidden md:block text-sm bg-gray-800 text-white px-3 py-1 rounded"
            >
              Admin
            </Link>
          )}
